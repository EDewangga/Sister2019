...
def listen_command():
    ...
    while alive:
        try:
            ...
            if req_split[0] == 'ls':
                res = server.get_list(req)
            elif req_split[0] == 'mk':
                res = server.create_file(req)
            elif req_split[0] == 'rm':
                res = server.delete_file(req)
            elif req_split[0] == 'cat':
                res = server.read_file(req)
            elif req_split[0] == 'update':
                res = server.update_file(req)
            elif req_split[0] == 'cd':
                res = server.connected_device_list()
            elif req_split[0] == 'exit':
                res = server.bye()
                alive = False
            elif req_split[0] == 'stop':
                res = server.stop_my_server()
            else:
